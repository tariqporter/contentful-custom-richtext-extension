...
const TestNode = ({ attributes, children }) => (
  <div style={{ border: '1px solid #f00' }} {...attributes}>
    {children}
  </div>
);
...
export class ConnectedRichTextEditor extends React.Component {
  ...
  // https://codesandbox.io/s/mjkrwv3py9?from-embed=&file=/index.js
  renderNode = (props, editor, next) => {
    // console.log(props.node.type)
    switch (props.node.type) {
      case 'embedded-entry-block': {
        // const [t1, t2] = props.node.data;
        // const test = toContentfulDocument(props.node);
        //sdk.space.getAsset
        // const node = props.node.toJSON();
        // const { id } = node.data.target.sys;
        // const asset = await this.props.sdk.space.getAsset(id);
        // console.log(props.node.type, id, asset);
        // console.log(props);
        return <TestNode {...props}>{next()}</TestNode>;
      }
      default: {
        return next();
      }
